...
def Z(Zparams, Zin):
    ...
    a = Zparams[5+6]
    return np.log(np.exp(a*G2) + np.exp(a*F2))/a

def Zcode(Zparams):
    # Fcodetext = Fcode(Zparams[:5])
    Fcodetext = Fcode(Fguess)
    Gcodetext = Gcode(Zparams[5:5+6])
    a = Zparams[5+6]
    return f'log( exp( {a:.3f}*({Gcodetext}) ) + exp( {a:+.3f}*({Fcodetext}) ) ) / {a:.3f}'
...
Zguess = np.array([1.104, 0.101, -0.924, 13.249, -4.144,     -0.12, 0.128, 0.240, 0.124, 0.18, -0.09, 0.1])
# Zguess = np.array([0.103,1.245,2.083,1.030,0.994]) # best found for the other model
Zsolutions = [Zguess + np.array([random.gauss(0,1) for j in range(len(Zguess))]) for i in range(50000)]
