...
def G(Gparams, Gin):
    return 1/(1+np.exp(-np.matmul(Gin,  Gparams)))
...
def Gcode(Gparams):
    ...
    a4 = (Gparams[4])
    a5 = (Gparams[5])
    return f'1/(1+exp(-({a0:.3f} {a1:+.3f}*(p/T) {a2:+.3f}*(1/T) {a3:+.3f}*(p/T)*(1/T) {a4:+.3f}*(p/T)**2  {a5:+.3f}*(1/T)**2) ))'
...
Zguess = np.array([1.037,0.106,-0.919,1.542,0.985,17.341,-7.712,-23.699,3.872,-0.404,9.205])
