...
def Zcode(Zparams):
    ...
    Icodetext = Icode(Zparams[5:5+6])
    return f'({Icodetext}) + (1-{Icodetext})*({Fcodetext})'
...
Zguess = np.array([1.104, 0.101, -0.924, 13.249, -4.144,      1.151, -0.477, -0.309, 0.194, -0.219, 0.123,      1.151, -0.477, -0.309, 0.194, -0.219, 0.123])
# Zguess = np.array([0.103,1.245,2.083,1.030,0.994]) # best found for the other model
Zsolutions = [Zguess + np.random.normal(0, 1, len(Zguess)) for i in range(1000000)]
